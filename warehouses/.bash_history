...
# For OS400
if $os400; then   COMMAND='chgjob job('$JOBNAME') runpty(6)';   system $COMMAND;    export QIBM_MULTI_THREADED=Y; fi
# Get standard Java environment variables
if $os400; then   . "$CATALINA_HOME"/bin/setclasspath.sh; else   if [ -r "$CATALINA_HOME"/bin/setclasspath.sh ]; then     . "$CATALINA_HOME"/bin/setclasspath.sh;   else     echo "Cannot find $CATALINA_HOME/bin/setclasspath.sh";     echo "This file is needed to run this program";     exit 1;   fi; fi
...
if [ -z "$LOGGING_CONFIG" ]; then   if [ -r "$CATALINA_BASE"/conf/logging.properties ]; then     LOGGING_CONFIG="-Djava.util.logging.config.file=$CATALINA_BASE/conf/logging.properties";   else     LOGGING_CONFIG="-Dnop";   fi; fi
